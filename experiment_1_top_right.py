...
parser.add_argument(
    "--save_dir",
    default=f"mSDA_pcv_{s_range_pcv}_{e_range_pcv}/Experiment_01_top_right",
)
    ...
    for index, t in np.ndenumerate(n_train_tasks):
        ...
        if p < 12:
            if s_hat.size > 0:
                ...
                results["shat"][rep, index] = mse(
                    lr_s_temp, x_test_subset_search[:, s_hat], y_test
                )
        ...
        if s_greedy.size > 0:
            ...
            results["sgreed"][rep, index] = utils.mse(
                lr_sg_temp, x_test[:, s_greedy], y_test
            )
        ...
        p_linsp = np.linspace(s_range_pcv, e_range_pcv, 10)
