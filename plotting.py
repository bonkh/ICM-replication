...
def plot_tl(file_name, ylim=None):
    ...
    for i, m in enumerate(methods):
        ax.plot(
            n_train_tasks + i * offset,
            (np.mean(np.log(results[m]), 0)),
            c=color_dict[m],
            alpha=0.5,
        )
        ax.errorbar(
    ...
    lgd = ax.legend(
        prop={"size": 15},
        bbox_to_anchor=(0.0, 1.08, 1.0, 0.102),
        ncol=4,
        loc="center",
        borderaxespad=0,
    )
...
def plot_mtl_mse(file_name):
    ...
    save_dir = "/".join(file_name.split("/")[0:-1])
    plt.savefig(
        os.path.join(save_dir, "plot_mse.pdf"),
        bbox_extra_artists=(lgd,),
        bbox_inches="tight",
        format="pdf",
        dpi=1000,
...
def plot_deltas(file_name):
    ...
    for i, m in enumerate(methods):
        ...
        if m == "pool" or m == "strue":
            ...
            dashed_std = np.array(
                xaxis.size
                * [1.96 / np.sqrt(n_repeat) * np.std(np.log(results[m]), 0)[0]]
            )

            ax.plot(
                xaxis, dashed_mean, c=color_dict[m], linestyle="--", label=legends[m]
            )
            ax.fill_between(
                xaxis,
                dashed_mean - dashed_std,
                dashed_mean + dashed_std,
                alpha=0.2,
...
def plot_mtl(file_name):
    ...
    for i, m in enumerate(methods):
        ...
        comp = np.sum((dom_av_tasks - av_tasks_method) > 0, 0).astype(int) / float(
            n_repeat
        )
        ax.plot(
            samp_array + i * offset,
    ...
    plt.ylabel(
        r"Percentage of simulations in" "\n" r"which $\beta^{dom}$ is outperformed",
        fontsize=15,
    )
...
def plot_mtl_ul(file_name):
    ...
    for i, m in enumerate(methods):
        ...
        comp = np.sum((cau_av_tasks - av_tasks_method) >= 0, 0).astype(int) / float(
            n_repeat
        )
        ax.plot(
            samp_array + i * offset,
    ...
    plt.ylabel(
        r"Percentage of simulations in"
        "\n"
        r"which $\beta^{CS(cau\sharp)}$ is outperformed",
        fontsize=15,
    )
...
def plot_interv(file_name):
    ...
    for i, l in enumerate(count):
        for p in range(count.shape[1]):
            ...
            ax.bar(
                i + (p - 3) * width, count[i, p], width - 0.02, color=col, alpha=0.35
            )
    ...
    ax.set_yticklabels(
        [r"$0$", r"$0.2$", r"$0.4$", r"$0.6$", r"$0.8$", r"$1$"], fontsize=15
    )

    plt.ylabel(
        r"Percentage of repetitions for which the"
        "\n"
    ...
    plt.savefig(
        os.path.join(save_dir, "plot.pdf"), bbox_inches="tight", format="pdf", dpi=1000
    )
...
def hist_mtl(file_name):
    ...
    plt.hist(
        dif, bins=30, facecolor="orange", alpha=0.5, edgecolor="black", linewidth=1.2
    )

    plt.xlabel(
        r"$\mathcal{E}(\beta^{dom})-\mathcal{E}(\beta^{cau\sharp})$", fontsize=15
    )
    plt.ylabel(r"Number of repetitions", fontsize=15)

    plt.savefig("mtl_hist.pdf", format="pdf", dpi=1000)
    plt.show()
