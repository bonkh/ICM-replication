        ...
        if fix_size > 0:
            for i in range(1, n_task):
                np.testing.assert_array_equal(
                    cov_n[i][-fix_size:, -fix_size:], cov_n[0][-fix_size:, -fix_size:]
                )
...
class TestDrawAll:
    ...
    def test_draw_all_basic(self):
        ...
        result = draw_all(
            alpha, n_task, n, p, p_s, p_conf, eps, g, lambd, beta_0, gamma_0
        )
...
class TestGaussTL:
    ...
    def test_gauss_tl_add_noise(self):
        ...
        assert tl.train["x_train"].shape == (
            original_shape[0],
            original_shape[1] + n_noise,
        )
        assert tl.train["x_test"].shape == (
            original_shape[0],
            original_shape[1] + n_noise,
        )
        assert tl.test["x_train"].shape == (
            original_shape[0],
            original_shape[1] + n_noise,
        )
        assert tl.test["x_test"].shape == (
            original_shape[0],
            original_shape[1] + n_noise,
        )
