...
parser.add_argument(
    "--save_dir", default=f"mSDA_pcv_{s_range_pcv}_{e_range_pcv}/Experiment_01_top_left"
)
...
for rep in range(n_repeat):
    ...
    for index, t in np.ndenumerate(n_train_tasks):
        ...
        # ************** 1. Pooled *********************
        results["pool"][rep, index] = train_linear_and_eval(
        ...
        if p < 10:
            if s_hat.size > 0:
                ...
                results["shat"][rep, index] = mse(lr_s_temp, x_test[:, s_hat], y_test)
...
file_name = "0.001_0.999" + "_".join(file_name)
