...
def mat_hsic(X, n_samples_per_task):
    ...
    for i in range(n_samples_per_task.size):
        domains[
            currentIndex : task_boundaries[i], currentIndex : task_boundaries[i]
        ] = np.ones((n_samples_per_task[i], n_samples_per_task[i]))
...
def full_search(
    ...
    if use_hsic:
        ...
        pvals = 1.0 - sp.stats.gamma.cdf(stat, a, scale=b)
    else:
...
def subset(
    ...
    train_x, train_y, valid_x, valid_y, n_ex_train, n_ex_valid = split_train_valid(
        x, y, n_samples_per_task_list, valid_split
    )
...
def greedy_search(
    train_x, train_y, valid_x, valid_y, n_ex, n_ex_valid, use_hsic, alpha, inc=0.0
):
    ...
    while stay == 1:
        ...
        if accepted[0].size > 0:
            ...
            selected[np.where(mse_a == best_mse)] = (
                selected[np.where(mse_a == best_mse)] + 1
            ) % 2
...
def greedy_subset(x, y, n_ex, delta, valid_split, use_hsic=False):
    ...
    """
    train_x, train_y, valid_x, valid_y, n_ex_train, n_ex_valid = split_train_valid(
        x, y, n_ex, valid_split
    )
    subset = greedy_search(
        train_x, train_y, valid_x, valid_y, n_ex_train, n_ex_valid, use_hsic, delta
    )
