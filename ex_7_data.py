...
def gen_coef(coef_0, lambd, mask=None):
    if not mask is None:
        ...
        # print(perturbation)
        ret[mask_compl] = coef_0[mask_compl] + lambd * perturbation[mask_compl]
...
def covs_all(n_task, p_s, p_n, mask=None, soft_strength=0.01):
    ...
    for k in range(n_task):
        ...
        if fix > 0:
            ...
            soft_intervene_cov = ref + soft_strength * soft_cov
...
def draw_tasks(n_task, n, params):
    ...
    return np.concatenate(x, 0), np.concatenate(y, 0), n_ex, eps * eps_draw
...
def draw_all(
    ...
    x, y, n_ex, noise = draw_tasks(n_task, n, params)
    x_test, y_test, n_ex_test, noise_test = draw_tasks(n_task, n, params)
    params['noise'] = noise
    params['noise_test'] = noise_test
...
class gauss_tl(object):
    ...
    def __init__(self, n_task, n, p, p_s, p_conf, eps, g, lambd, lambd_test, mask=None):
        ...
        alpha = np.array([1.5] * p_s).reshape(-1, 1)

        # gamma_0 = np.random.normal(0, 1, (p_n, 1))
        gamma_0 = np.array([[0.7], [0.7], [0.7]])
        ...
        self.test = {}
        self.test['noise'] = params['noise_test']
